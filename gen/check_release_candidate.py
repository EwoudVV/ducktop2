...
ACTIVE_BOARDS = (
    "ducktop2-center.kicad_pcb", "left_io/left_io.kicad_pcb",
    "right_io/right_io.kicad_pcb", "bms/bms.kicad_pcb",
)
NUMBER = r"[-+0-9.eE]+"
...
def select_pcbs(stage: str, requested: Path | None) -> list[Path]:
    ...
    names = ACTIVE_BOARDS if stage in {"fabrication", "production"} else ACTIVE_BOARDS[:1]
...
def schematic_for_board(pcb: Path, requested: Path | None = None) -> Path:
    if requested is not None:
        return requested.expanduser().resolve()
    if pcb.resolve() == DEFAULT_PCB.resolve():
        return DEFAULT_SCHEMATIC.resolve()
...
def run_pcb_checks(cli: str, pcb: Path, tempdir: Path,
                   schematic: Path | None = None, reports: Path | None = None) -> int:
    failures = 0
    ...
    print(f"Unconnected findings listed in DRC: {len(unconnected)}")
    ...
    extra_unconnected = max(0, len(refilled.get("unconnected_items", [])) - len(unconnected))
                ...
                for _pcb in pcbs:
                    try:
                        failures += run_pcb_checks(cli, _pcb, tempdir,
                                                  ...
                                                  reports / _pcb.stem if reports else None)
