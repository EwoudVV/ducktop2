...
def add_pd_selector(s):
    ...
    # Phase 5: the LTC4418IUF symbol pinout was corrected (6=VALID1,
    # 7=VALID2, 8=GND, 9=CAS, 10=INTVCC -- see generate_power_sheet.py).
    # Phase 5: the V2 channel (PD2) moved to the center board (U15B);
    # PD2_VBUS_GATED now crosses FPC-2, so U14 runs single-input with
    # V2 grounded per the datasheet ("Connect V2 to ground when channel
    # is not used").  PD2 validity is generated by U15B on the center.
    s.place("U14", "LTC4418IUF", "LTC4418IUF#PBF dual-input PD selector (V1 = J21)", 116.84, 574.04,
            footprint=FOOTPRINTS["LTC4418IUF"], pin_nets={
                "1": ("PD_SEL_TMR", "local"), "2": ("PD1_SEL_UV", "local"),
                "3": ("PD1_SEL_OV", "local"), "4": ("GND", "local"),
                "5": ("GND", "local"), "6": ("PD1_VALID_N", "hier"), "7": ("", "nc"),
                "8": ("GND", "local"), "9": ("", "nc"), "10": ("PD_SEL_INTVCC", "local"),
                "11": ("", "nc"),
                "12": ("GND", "local"), "13": ("PD1_SEL_GATE", "local"),
                "14": ("PD1_SEL_FET_COMMON", "local"), "15": ("USB_PD_SELECTED", "hier"),
                "16": ("GND", "local"), "17": ("PD1_VBUS_GATED", "local"),
    ...
    s.text(20.32, 685.8,
           "Phase 5: J11 (PD2) charge power is selected on the CENTER board (U15B); this board exports its own gated VBUS only.")
