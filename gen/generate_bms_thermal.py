...
def add_bms_thermal(s):
    ...
    def cap(ref,value,x,y,a,z=RAW,mpn=C100,footprint=CFP):
        return part(ref,'C',value,x,y,footprint,{'1':a,'2':z},mpn,
                    'TDK' if mpn.startswith(('CGA','C1005')) else 'Murata')
    ...
    for index,(name,top,mpn) in enumerate([
        ...
        ref_cap_mpn = C10N_0402 if name == 'DSG_HOT' else C10N
        ref_cap_fp = 'Capacitor_SMD:C_0402_1005Metric' if name == 'DSG_HOT' else CFP
        cap(f'C{2240+index}','10n 50V thermal reference filter',x,490,net,
            mpn=ref_cap_mpn,footprint=ref_cap_fp)
    ...
    for cell in range(1,4):
        ...
        # The third sense resistor leaves space for the reference-filter via.
        sense_mpn = 'RT0402BRD0710KL' if cell == 3 else 'RT0603BRD0710KL'
        sense_fp = 'Resistor_SMD:R_0402_1005Metric' if cell == 3 else RFP
        resistor(f'R{2211+(cell-1)*3}','10k 0.1% 25ppm NTC sense-lead limit',x,325,
                 sense,wire_a,sense_mpn,footprint=sense_fp)
    ...
    resistor('R2254','1k local retry gate series',360,670,
             'PACK_RETRY_LOCAL','PACK_RETRY_LOCAL_GATE','ERJ2RKF1001X',
             footprint='Resistor_SMD:R_0402_1005Metric',manufacturer='Panasonic')
