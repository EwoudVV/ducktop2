...
static bool latch, fail_write, expect_off_before_flags;
...
bool i2c1_write(uint8_t address,uint8_t reg,uint8_t value)
{
    ...
    io[reg]=value;
    if (reg==3 && !(value&1u) && !(ina[6]&0x10u)) latch=false;
    pins();return true;
}
...
int main(void)
{
    ...
    assert(usb_power_alert_threshold_raw()==12284u);
    ...
    assert(log_items[6].bytes[0]==7 && log_items[6].bytes[1]==0x2f && log_items[6].bytes[2]==0xfc);
